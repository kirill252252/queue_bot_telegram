...
def _notifications_enabled(queue: dict) -> bool:
    """Проверяет, разрешил ли администратор уведомления для очереди."""
    return _as_bool(queue.get("notifications_enabled", True))
...
async def safe_dm(bot: Bot, user_id: int, text: str, **kwargs) -> bool:
    try:
        ...
        logger.info("Cannot DM %s: %s", user_id, e)
        return False
...
async def notify_became_first(bot: Bot, queue: dict, member: dict, chat_id: int):
    if not _notifications_enabled(queue):
        return
    ...
    await db.create_reminder(queue["id"], member["user_id"], fire_at)


async def notify_approaching(bot: Bot, queue: dict, member: dict, position: int):
    if NOTIFY_APPROACHING <= 0 or position > NOTIFY_APPROACHING:
        return
    if not _notifications_enabled(queue):
        return
...
async def notify_slot_available(bot: Bot, queue: dict):
    # Для безлимитных очередей подписки не нужны — всегда можно войти
    ...
    for user_id in subscribers:
        ...
        if slots_left <= 0:
            break  # очередь снова заполнилась, остальным не отправляем

        kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="✋ Занять место", callback_data=f"pm_join:{queue['id']}")
        ]])
        sent = await safe_dm(
            bot, user_id,
...
async def process_due_reminders(bot: Bot):
    ...
    for r in reminders:
        ...
        member = members[0]
        auto_kick = _as_bool(queue.get("auto_kick", True))
