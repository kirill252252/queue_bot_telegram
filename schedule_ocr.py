...
ПРАВИЛО 1 — РАЗНЫЕ ЦИФРЫ В ЛЕНТЕ = РАЗНЫЕ ПАРЫ:
  Если слева стоят цифры 1, 2, 3 — это ТРИ разные пары.
  Даже если у лент 1 и 2 ОДИНАКОВЫЙ текст — создай ДВЕ отдельные ячейки!

ПРАВИЛО 2 — ОДНА ЦИФРА + ДВЕ СТРОКИ = РАЗДЕЛ ПО НЕДЕЛЯМ:
  Если одна цифра лента, а внутри ячейки ДВЕ строки:
  - Верхняя строка → нечётная неделя (odd)
  - Нижняя строка → чётная неделя (even)
  Это week_mode = "odd_even"

ПРАВИЛО 3 — ПУСТАЯ ПОЛОВИНА:
  - Пустой/прочерк верх + заполненный низ → week_mode = "even_only"
  - Заполненный верх + пустой/прочерк низ → week_mode = "odd_only"
  - Одна строка без разделения → week_mode = "every_week"

ПРАВИЛО 4 — МЕРОПРИЯТИЯ:
  «Разговоры о важном» / «Внеклассное мероприятие» → lesson_num=0, is_event=1, week_mode="event"

═══════════════ КОНКРЕТНЫЕ ПРИМЕРЫ ═══════════════

ПРИМЕР 1: Вторник с двумя одинаковыми парами подряд
...
ПРИМЕР 3: Среда — лента 1 только чётная, лента 5 только нечётная
