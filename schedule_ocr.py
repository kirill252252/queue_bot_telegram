...
Таблица имеет колонки слева направо:
  1. «День» — день недели написан ВЕРТИКАЛЬНО: ПОНЕДЕЛЬНИК=1, ВТОРНИК=2, СРЕДА=3, ЧЕТВЕРГ=4, ПЯТНИЦА=5
  2. «дата» — дата (игнорируй)
  3. «Ауд.» — номер аудитории (room)
  4. Содержимое ячейки: название предмета и преподаватель

В колонке слева от аудитории стоит ЦИФРА ЛЕНТА — номер пары (1–6).
Weekday читай из вертикальной подписи «День», а НЕ из цифр лент!

══════════════ ⚠️ КРИТИЧЕСКОЕ ПРАВИЛО ⚠️ ═══════════════
Номер лента определяется ТОЛЬКО по явной цифре в левой колонке.

  ОДНА ЦИФРА + ДВЕ СТРОКИ ТЕКСТА = ОДНА ЯЧЕЙКА, РАЗДЕЛЁННАЯ ПО НЕДЕЛЯМ:
  • верхняя строка → нечётная неделя (odd)
  • нижняя строка → чётная неделя (even)
  → week_mode = "odd_even"

  Если рядом с текстом НЕТ явной новой цифры — этот текст является
  НИЖНЕЙ ПОЛОВИНОЙ предыдущей ленты (чётная неделя), а не новой парой!

═══════════════ ОСТАЛЬНЫЕ ПРАВИЛА ═══════════════
• Одна строка → week_mode = "every_week"
• Верх пустой/прочерк + низ заполнен → week_mode = "even_only"
• Верх заполнен + низ пустой/прочерк → week_mode = "odd_only"
• «Разговоры о важном» / «Внеклассное мероприятие» → lesson_num=0, is_event=1, week_mode="event"

══════════════ ПРИМЕР (Понедельник П-5-24) ══════════════
Таблица выглядит так (лента | ауд. | текст):
  0  |       | Внеклассное мероприятие «Разговоры о важном» / Табаченко И.К.   ← одна строка, lesson_num=0, event
  1  | 509   | Инструментальные средства разработки / Наприенко ЕМ             ← одна строка → every_week
  2  | 509   | Инструментальные средства разработки / Наприенко ЕМ             ← ДВЕ строки в одной ячейке:
     | 410   | Программирование web-приложений / Вахитов РГ                    ← нет новой цифры → это нижняя половина лента 2!
  3  | 212   | Дискретная математика / Бронникова ЕН                           ← одна строка → every_week
  4  |       | (пусто)

→ Создаёт ЧЕТЫРЕ ячейки (НЕ ПЯТЬ!):
...
══════════════ ПРИМЕР (Среда П-5-24) ══════════════
  1  |       | (прочерк / пусто)                                 ← верх пуст → even_only
     | 404   | Математическое моделирование / Мережникова ЕМ     ← нижняя половина лента 1
  2  | 410   | Системное программирование / Гвоздиевская ОС      ← ДВЕ строки → odd_even
     | 410   | Технология разработки программного обеспечения / Гвоздиевская ОС
  3  | 410   | Системное программирование / Гвоздиевская ОС      ← одна строка → every_week
  4  | С/3   | Физическая культура / Бахирева НА                 ← одна строка → every_week
  5  | Л610  | Иностранный язык в проф. деятельности / Данилова АА ← верх заполнен, низ пуст → odd_only
...
→ Создаёт ПЯТЬ ячеек:
  ...
  {"weekday":3, "lesson_num":2, "week_mode":"odd_even",
   "top":   {"subject":"Системное программирование","teacher":"Гвоздиевская ОС","room":"410"},
   "bottom": {"subject":"Технология разработки программного обеспечения","teacher":"Гвоздиевская ОС","room":"410"}}
...
Верни ТОЛЬКО JSON (без пояснений):
{
  "groups": [
    {
      "group_name": "П-5-24",
      "cells": [ ...все ячейки всех дней... ]
    }
  ]
}

Допустимые week_mode: "every_week", "odd_even", "odd_only", "even_only", "event", "empty"
top/bottom: null если отсутствует.
Ошибка распознавания → {"error":"не удалось распознать"}
"""
