...
Если в слоте только ОДНА строка текста — СОХРАНЯЙ week_type как есть (0=каждую, 1=нечётная, 2=чётная).
...
4. Если верхняя половина ячейки пустая/прочерк, а нижняя содержит предмет, это только week_type 2 (чётная).
5. Если нижняя половина ячейки пустая/прочерк, а верхняя содержит предмет, это только week_type 1 (нечётная).
...
9. ОДИНОЧНЫЕ пары (одна строка в ячейке) сохраняй их week_type как есть — не меняй на 0!
...
═══════════════ СТРУКТУРА ТАБЛИЦЫ ═══════════════
Таблица: День (вертикально) | дата | Ауд. | Предмет + преподаватель
Слева от аудитории — колонка ЛЕНТА: цифра = номер пары (1–6).
Weekday из вертикального текста: ПОНЕДЕЛЬНИК=1 ВТОРНИК=2 СРЕДА=3 ЧЕТВЕРГ=4 ПЯТНИЦА=5

══════════ ПРАВИЛО ЛЕНТА vs РАЗДЕЛЕНИЕ ПО НЕДЕЛЯМ ══════════

Смотри на колонку ЛЕНТА (левая, с цифрами). Для каждой СТРОКИ — есть ли там явная цифра?

  А) Явная цифра "2" стоит в колонке для КАЖДОЙ строки → каждая строка это ОТДЕЛЬНАЯ пара:
     Строка с "1" = пара 1 (every_week)
     Строка с "2" = пара 2 (every_week)
     ⚠️ ДАЖЕ ЕСЛИ ПРЕДМЕТЫ ОДИНАКОВЫЕ — это ДВЕ разные пары!

  Б) Явная цифра стоит только у ПЕРВОЙ строки, у второй строки цифры НЕТ (пусто) →
     обе строки — это ОДНА пара, разделённая по неделям:
     Первая строка (верхняя визуально) → "top" → нечётная неделя (week 1)
     Вторая строка (нижняя визуально) → "bottom" → чётная неделя (week 2)
     → week_mode = "odd_even"

  В) Верхняя строка пуста/прочерк/тире (---) + нижняя заполнена → week_mode = "even_only"
  Г) Верхняя заполнена + нижняя пуста/прочерк/тире (---) → week_mode = "odd_only"
  Д) Одна строка → week_mode = "every_week"
  Е) «Разговоры о важном» / «Внеклассное» → lesson_num=0, is_event=1, week_mode="event"

════════ ПРИМЕР 1: Вторник (ДВЕ одинаковые пары с РАЗНЫМИ цифрами лента) ════════
  Колонка ЛЕНТА:
  1  | С-6  | Разработка программных модулей — Петрова АА   ← цифра "1" явно стоит
  2  | С-6  | Разработка программных модулей — Петрова АА   ← цифра "2" явно стоит
  3  | 303  | История России — Андриевская НМ

→ ТРИ ячейки (НЕ ДВЕ!), одинаковый предмет не мешает:
  {"weekday":2, "lesson_num":1, "week_mode":"every_week", "top":{"subject":"Разработка программных модулей","teacher":"Петрова АА","room":"С-6"}, "bottom":null}
  {"weekday":2, "lesson_num":2, "week_mode":"every_week", "top":{"subject":"Разработка программных модулей","teacher":"Петрова АА","room":"С-6"}, "bottom":null}
  {"weekday":2, "lesson_num":3, "week_mode":"every_week", "top":{"subject":"История России","teacher":"Андриевская НМ","room":"303"}, "bottom":null}

════════ ПРИМЕР 2: Понедельник (лента 2 разделена по неделям) ════════
  Колонка ЛЕНТА:
  0  |      | Внеклассное мероприятие «Разговоры о важном» / Табаченко И.К.
  1  | 509  | Инструментальные средства разработки — Наприенко ЕМ   ← цифра "1", одна строка
  2  | 509  | Инструментальные средства разработки — Наприенко ЕМ   ← цифра "2", ДВЕ строки (нет новой цифры ниже)
     | 410  | Программирование web-приложений — Вахитов РГ          ← НЕТ цифры = нижняя половина лента 2
  3  | 212  | Дискретная математика — Бронникова ЕН                 ← цифра "3", одна строка

→ ЧЕТЫРЕ ячейки:
  {"weekday":1, "lesson_num":0, "week_mode":"event", "top":{"subject":"Внеклассное мероприятие «Разговоры о важном»","teacher":"Табаченко И.К.","room":""}, "bottom":null}
  ...
  {"weekday":1, "lesson_num":3, "week_mode":"every_week", "top":{"subject":"Дискретная математика с элементами математической логики","teacher":"Бронникова ЕН","room":"212"}, "bottom":null}

════════ АНТИ-ПАТТЕРН (Понедельник — ловушка одинакового текста) ════════
  Колонка ЛЕНТА:
  1  | 509  | Инструментальные средства разработки — Наприенко ЕМ   ← цифра "1", ОДНА строка → every_week
  2  | 509  | Инструментальные средства разработки — Наприенко ЕМ   ← цифра "2" явно есть! ДВЕ строки
     | 410  | Программирование web-приложений — Вахитов РГ          ← нет цифры = нижняя половина лента 2

  ⚠️ Хотя строки "1" и верх "2" выглядят одинаково — лента 1 и лента 2 это РАЗНЫЕ пары!
  ⚠️ НЕ объединяй лента 1 с нижней строкой лента 2 ("Прогр.web") в split!
  ⚠️ Признак split — нижняя строка БЕЗ цифры лента. Строка с явной "2" — НАЧАЛО НОВОЙ ПАРЫ.

→ ПРАВИЛЬНО: лента 1 = every_week, лента 2 = odd_even
  {"weekday":1, "lesson_num":1, "week_mode":"every_week", "top":{"subject":"Инструментальные средства разработки программного обеспечения","teacher":"Наприенко ЕМ","room":"509"}, "bottom":null}
  {"weekday":1, "lesson_num":2, "week_mode":"odd_even",
   "top":    {"subject":"Инструментальные средства разработки программного обеспечения","teacher":"Наприенко ЕМ","room":"509"},
   "bottom": {"subject":"Программирование web-приложений","teacher":"Вахитов РГ","room":"410"}}

→ НЕПРАВИЛЬНО (распространённая ошибка!):
  {"lesson_num":1, "week_mode":"odd_even", "top":{"subject":"Инстр...","room":"509"}, "bottom":{"subject":"Прогр.web...","room":"410"}}
  ← ОШИБКА: лента 2 пропала, лента 1 незаконно стала split-ячейкой!

════════ ПРИМЕР 4: Четверг (три одинаковые пары с явными цифрами) ════════
  Колонка ЛЕНТА:
  1  | 417  | Психология общения — Чепенко СА         ← цифра "1"
  2  | С-6  | Разработка программных модулей — Петрова АА  ← цифра "2" явно есть
  3  | С-6  | Разработка программных модулей — Петрова АА  ← цифра "3" явно есть (такой же текст — это норма!)
  4  | 220  | Математическое моделирование — Мережникова ЕМ ← цифра "4"

→ ЧЕТЫРЕ ячейки (НЕ ТРИ!), лента 2 и 3 — отдельные пары несмотря на одинаковый текст:
  {"weekday":4, "lesson_num":1, "week_mode":"every_week", "top":{"subject":"Психология общения","teacher":"Чепенко СА","room":"417"}, "bottom":null}
  {"weekday":4, "lesson_num":2, "week_mode":"every_week", "top":{"subject":"Разработка программных модулей","teacher":"Петрова АА","room":"С-6"}, "bottom":null}
  {"weekday":4, "lesson_num":3, "week_mode":"every_week", "top":{"subject":"Разработка программных модулей","teacher":"Петрова АА","room":"С-6"}, "bottom":null}
  {"weekday":4, "lesson_num":4, "week_mode":"every_week", "top":{"subject":"Математическое моделирование","teacher":"Мережникова ЕМ","room":"220"}, "bottom":null}
  Колонка ЛЕНТА:
  1  |      | ----------------------   ← цифра "1", верхняя строка = ПРОЧЕРКИ (нет пары)
     | 404  | Математическое моделирование — Мережникова ЕМ  ← нет цифры = нижняя половина лента 1
     ⚠️ Верх = прочерки = пусто → week_mode = "even_only" (только чётная неделя)
  2  | 410  | Системное программирование — Гвоздиевская ОС   ← цифра "2", ДВЕ строки
     | 410  | Технология разработки программного обеспечения — Гвоздиевская ОС  ← нет цифры
     ⚠️ Обе строки заполнены → week_mode = "odd_even"
  3  | 410  | Системное программирование — Гвоздиевская ОС   ← цифра "3", одна строка → every_week
  4  | С/3  | Физическая культура — Бахирева НА              ← цифра "4", одна строка → every_week
  5  | Л610 | Иностранный язык в проф. деятельности — Данилова АА  ← цифра "5", ДВЕ строки
     |      | ----------------------   ← нет цифры, нижняя строка = ПРОЧЕРКИ (нет пары)
     ⚠️ Низ = прочерки = пусто → week_mode = "odd_only" (только нечётная неделя)
...
→ ПЯТЬ ячеек:
  ...
  {"weekday":3, "lesson_num":2, "week_mode":"odd_even",   "top":{"subject":"Системное программирование","teacher":"Гвоздиевская ОС","room":"410"}, "bottom":{"subject":"Технология разработки программного обеспечения","teacher":"Гвоздиевская ОС","room":"410"}}
  {"weekday":3, "lesson_num":3, "week_mode":"every_week", "top":{"subject":"Системное программирование","teacher":"Гвоздиевская ОС","room":"410"}, "bottom":null}
  {"weekday":3, "lesson_num":4, "week_mode":"every_week", "top":{"subject":"Физическая культура","teacher":"Бахирева НА","room":"С/3"}, "bottom":null}
...
═══════════════ ФОРМАТ ОТВЕТА ═══════════════
СНАЧАЛА выполни самопроверку — посчитай для каждого дня явные цифры лента:
  "visible_lenta": {"1": [0,1,2,3], "2": [1,2,3], "3": [1,2,3,4,5], "4": [1,2,3,4], "5": [1,2,3]}
Затем проверь: количество уникальных lesson_num в cells для дня D == длина visible_lenta[D].
Если не совпадает — найди и исправь ошибку перед выводом.

Верни ТОЛЬКО JSON (с полем visible_lenta):
{"visible_lenta":{"1":[0,1,2,3],"2":[1,2,3],"3":[1,2,3,4,5],"4":[1,2,3,4],"5":[1,2,3]},"groups":[{"group_name":"...","cells":[{"weekday":1,"lesson_num":1,"week_mode":"every_week","is_event":0,"top":{"subject":"...","teacher":"...","room":"..."},"bottom":null}]}]}

week_mode: "every_week" | "odd_even" | "odd_only" | "even_only" | "event" | "empty"
top/bottom: null если отсутствует.
Ошибка → {"error":"не удалось распознать"}
"""
...
═══════════════ ПРАВИЛА ═══════════════
1. РАЗНЫЕ ЦИФРЫ СЛЕВА = РАЗНЫЕ ПАРЫ, даже если текст одинаковый. НЕ ОБЪЕДИНЯЙ!
   Лента 1 = "Предмет А" и Лента 2 = "Предмет А" → ДВЕ ячейки с lesson_num 1 и 2.
2. ОДНА ЦИФРА + ДВЕ СТРОКИ ВНУТРИ = деление по неделям:
   - Верх = нечётная неделя (odd)
   - Низ  = чётная неделя (even)
   → week_mode = "odd_even"
3. Верх заполнен, низ пустой/прочерк → "odd_only"
4. Верх пустой/прочерк, низ заполнен → "even_only"
5. Одна обычная строка (без разделения) → "every_week"
...
def _fix_lesson_gaps(cells: list, visible_lenta: dict) -> list:
    """
    Исправляем пропущенные пары используя visible_lenta из самопроверки модели.

    Симптом: модель читает лента N (every_week, предмет X) + лента N+1 top (тот же X)
    как ОДНУ split-ячейку N. В результате N+1 пропадает, а N становится odd_even.

    Лечение: если visible_lenta говорит что должна быть пара M, но в cells её нет,
    и ячейка M-1 является odd_even — разделяем M-1 на every_week(M-1) + odd_even(M).
    """
    if not visible_lenta:
        return cells

    for wd_key, expected in visible_lenta.items():
        try:
            weekday = int(wd_key)
        except (ValueError, TypeError):
            continue

        expected_set = set(int(x) for x in expected if str(x).isdigit() or isinstance(x, int))
        actual_set = set(
            int(c["lesson_num"])
            for c in cells
            if int(c.get("weekday", 0)) == weekday
        )
        ...
        for miss in missing:
            # Ищем предыдущую ячейку (miss-1) которая odd_even — вероятная жертва схлопывания
            prev = next(
                (c for c in cells
                 if int(c.get("weekday", 0)) == weekday
                 and int(c.get("lesson_num", -1)) == miss - 1
                 and c.get("week_mode") == "odd_even"),
                None,
            )
            if prev is None:
                logger.debug("_fix_lesson_gaps: wd=%d miss=%d, no odd_even at %d", weekday, miss, miss - 1)
                continue

            top = prev.get("top")
            bottom = prev.get("bottom")

            # Предыдущая ячейка на самом деле была every_week
            prev["week_mode"] = "every_week"
            prev["bottom"] = None

            # Пропущенная пара — это настоящий split
            cells.append({
                "weekday": weekday,
                "lesson_num": miss,
                "week_mode": "odd_even",
            ...
            logger.info(
                "_fix_lesson_gaps: wd=%d restored lesson_num=%d (split from %d)",
                weekday, miss, miss - 1,
            )
...
def _expand_schedule_cells(result: dict) -> Optional[dict]:
    ...
    expanded_groups = []
    visible_lenta_raw = result.get("visible_lenta") or {}
    # Нормализуем ключи: {"1": [...]} или {"weekday_1": [...]}
    visible_lenta: dict[int, list] = {}
    for k, v in visible_lenta_raw.items():
        try:
            wd = int(str(k).replace("weekday_", ""))
            visible_lenta[wd] = v
        except (ValueError, TypeError):
            pass
    ...
    for group in groups:
        ...
        raw_cells = [c for c in group.get("cells", []) if isinstance(c, dict)]
        # Добавляем weekday в каждую ячейку (если не было) и исправляем пробелы
        fixed_cells = _fix_lesson_gaps(raw_cells, visible_lenta)

        lessons = []
        for raw_cell in fixed_cells:
            ...
            if week_mode == "every_week":
                ...
                part = top or bottom
                if part:
                    lessons.append(_make_lesson_from_part(weekday, lesson_num, part, 1, 0))
            elif week_mode == "even_only":
                part = bottom or top
                if part:
                    lessons.append(_make_lesson_from_part(weekday, lesson_num, part, 2, 0))
...
def _correct_week_types_after_flat_ocr(result: dict) -> dict:
    """
    Исправляет ошибки деления на недели при flat-OCR (SCHEDULE_PROMPT).

    ИСПРАВЛЕНО v2:
    - Одиночная запись в слоте → НЕ меняем week_type (может быть odd_only/even_only)
    - Две записи в слоте → присваиваем 1 и 2 В ПОРЯДКЕ ВОЗВРАТА МОДЕЛЬЮ (не по алфавиту!)
      Модель возвращает пары в порядке чтения: верхняя (нечётная) первой → [0]=week1, [1]=week2
    - Три и более в слоте → не трогаем
    """
    ...
    for group in result.get("groups", []):
        ...
        # Группируем по (weekday, lesson_num), сохраняя порядок
        by_slot: dict = {}
        slot_order: list = []
        for l in lessons:
            key = (int(l.get("weekday") or 0), int(l.get("lesson_num") or 0))
            if key not in by_slot:
                by_slot[key] = []
                slot_order.append(key)
            by_slot[key].append(l)

        fixed = []
        for key in slot_order:
            slot_lessons = by_slot[key]

            if len(slot_lessons) == 1:
                # ✅ ИСПРАВЛЕНО: НЕ трогаем week_type одиночной записи.
                # Одиночная пара в слоте могла быть odd_only или even_only — это правильно.
                # Только если week_type=0 — оставляем как есть (каждую неделю).
                fixed.append(slot_lessons[0])

            elif len(slot_lessons) == 2:
                # ✅ ИСПРАВЛЕНО: Используем порядок модели, а не алфавитный.
                # Модель возвращает верхнюю строку первой (нечётная), нижнюю второй (чётная).
                slot_lessons[0]["week_type"] = 1  # нечётная
                slot_lessons[1]["week_type"] = 2  # чётная
                fixed.extend(slot_lessons)

            else:
                # Три и более — оставляем как есть
                fixed.extend(slot_lessons)
